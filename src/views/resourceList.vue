<template>
  <section class="mt-5">
    ...
    <p v-for="(resourceItem,i) in resources" :key="i" class="row" v-if="!spinner">
      ...
      <router-link :to="`update/${resourceItem.id}`" class="btn btn-primary col-offet-1 mx-2" append>{{ $t('buttons.edit') }}&nbsp;&nbsp;<i class="fas fa-pencil-alt"></i></router-link>
      <button class="btn btn-danger  col-offet-1 mx-2" @click="remove(resourceItem.id)">{{ $t('buttons.remove') }}&nbsp;&nbsp;<i class="fas fa-trash-alt"></i></button>
    </p>
  </section>
</template>
